<?php
  ...
  $sql = "SELECT a.id_usulan, a.keterangan, b.id_posisi, d.jenis_posisi, a.tgl_usulan, a.masa_penilaian_awal, a.masa_penilaian_akhir, a.status_proses, b.nip, b.nm_lengkap, d.nm_posisi,
(SELECT SUM(IFNULL(aa.angka_kredit, 0)) FROM tbl_usulan_unsur aa WHERE aa.id_usulan = a.id_usulan) AS angka_kredit_diusulkan,
            (SELECT SUM(CASE WHEN aa.angka_kredit_baru = 0 THEN aa.angka_kredit ELSE aa.angka_kredit_baru END) FROM tbl_usulan_unsur aa WHERE aa.id_usulan = a.id_usulan AND aa.status = 'Diterima') AS angka_kredit_diterima FROM tbl_usulan a 
JOIN tbl_pegawai b ON a.nip = b.nip join tbl_posisi d ON b.id_posisi = d.id_posisi WHERE 1";
  $where = [];
  ...
  $sql_atasan = "SELECT a.nip, a.nm_lengkap FROM tbl_pegawai a JOIN tbl_posisi b ON a.id_posisi = b.id_posisi WHERE b.jenis_posisi = 'Pejabat Pengusul'";
  $atasan = $db->query($sql_atasan)->fetchAll(PDO::FETCH_ASSOC);
  
  $sql_atasan = "SELECT a.nip, a.nm_lengkap FROM tbl_pegawai a JOIN tbl_posisi b ON a.id_posisi = b.id_posisi WHERE a.is_atasan = 1";
  $atasan_pegawai = $db->query($sql_atasan)->fetchAll(PDO::FETCH_ASSOC);
?>
...
<div class="wrapper" style="height: auto; min-height: 100%;">
          ...
          <div class="table-responsive" style="overflow-x: visible; overflow-y:visible;">
            ...
            ?>
                ...
                <?php endif; ?>
                  ...
                  <td><?=$d['keterangan']?></td>
                  <td>
                    <?php if($_SESSION['is_atasan'] == 1 || $_SESSION['jenis_posisi'] == "Pejabat Pengusul"): ?>
                    ...
                    <div class="dropdown">
                      ...
                      <ul class="dropdown-menu">
                        ...
                        <?php endif; ?>
                        <?php if($_SESSION['jenis_posisi'] == "Pejabat Pengusul"): ?>
                          <li><a href="#" onclick="cetakDupak(<?=$d['id_usulan'].",'".$d['nip']."'"?>)">Cetak DUPAK</a></li>
          ...
          <form action="<?=$alamat_web?>/usulan/cetak-spmk.php">
            ...
            <div class="form-group">
              ...
              <select class="form-control" name="nip_atasan">
                <?php foreach($atasan_pegawai as $d): ?>
  ...
  <div class="modal fade" id="cetak-pengantar" style="display: none;">
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
        ...
        <div class="modal-body">
          <form action="<?=$alamat_web?>/usulan/cetak-pengantar.php">
            <input type="hidden" name="id_usulan" />
            <input type="hidden" name="nip" value="<?=$_SESSION['nip']?>" />
            <div class="form-group">
              <label>Pilih Pejabat Pengusul</label>
  ...
  <script>
    function cetakSpmk(id_usulan, nip){
      document.getElementsByName("id_usulan")[0].value = id_usulan;
      document.getElementsByName("nip")[0].value = nip;
      
      $('#cetak-spmk').modal('show');
    }
    function cetakDupak(id_usulan, nip){
      document.getElementsByName("id_usulan")[1].value = id_usulan;
      document.getElementsByName("nip")[1].value = nip;
      $('#cetak-dupak').modal('show');
    }
    function cetakSuratPengantar(id_usulan, nip){
      document.getElementsByName("id_usulan")[2].value = id_usulan;
      document.getElementsByName("nip")[2].value = nip;
      $('#cetak-pengantar').modal('show');
    }
  </script>
</div>
</body>
